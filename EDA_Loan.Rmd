...
names(ld)[16]<-paste("ListingCategory")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=12}
ggplot(data = ld_clean, aes(x = DebtToIncomeRatio, y = reorder(Occupation, desc(Occupation)), color = ProsperScore)) +
  geom_point(alpha = 0.25, size = 1, position = 'jitter') +
  labs(x = "Razão entre Dívida e Renda Mensal", y = "Profissão", title = "Razão entre Dívida e Renda Mensal por Profissão por PorsperScore") +
