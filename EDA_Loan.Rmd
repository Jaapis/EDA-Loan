...
ggplot(filter(ld_clean, CurrentDelinquencies > 2), aes(CurrentDelinquencies)) +
  geom_histogram()+
  scale_x_continuous(limits = c(2,18), breaks = seq(0,18,2)) +
...
qplot(AmountDelinquent, data = ld_clean)
...
summary(log10(ld_clean$AmountDelinquent + 1))

qplot(log10(AmountDelinquent), data = ld_clean) +
  ggtitle("Log10(AmountDelinquent)")

```

Fazendo uma filtragem dos dados com log10 apresenta-se os seguintes dados.
